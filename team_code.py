    ...
    def train_model(leads):
        ...
        def indices(leads):
            if leads==twelve_leads:
                ...
                pos = [0,1,2,3,4,5,12,13,14,15,16,17]
            elif leads==four_leads:
                pos = [0,1,2,7,12,13,14,19]
...
def run_model(model, header, recording):
    ...
    def generate_image(recording,frequency):
        recording = recording[:,:20*frequency]
        fftsignals = rfft(recording)
        fftsignals = fftsignals[:,:1024]
        mag = np.absolute(fftsignals)
    ...
    image = np.array([generate_image(recording,frequency)])
    probabilities = model.predict(image)
    probabilities = probabilities.reshape(26)
    labels = (probabilities>0.2)*1
...
def create_model(leads):
    ...
    model = Sequential()
    model.add(Conv1D(filters=64, kernel_size=7, activation='relu', input_shape=(1024,2*len(leads))))
